...
void BucketSort::attach(Segment &tile) {}

void BucketSort::apply(Segment &segment) {
    ...
    // copy sorted pixels back into segment
    for (int i = 0; i < segment.size(); i++)
        segment.setPixel(i, traversal, skew, sorted[i]);
}

std::unique_ptr<Effect> BucketSort::clone() {
